export const calculateWPM = (correctChars: number, totalTime: number = 10000): number => { 
    const wpm = Math.round((correctChars / 5 / totalTime) * 60);

    return wpm < 0 || wpm === Infinity ? 0 : wpm;
}

export const calculateCPM = (correctChars: number, totalTime: number = 10000): number => {
